...
D3DCOLOR lightSteelBluet = D3DCOLOR_ARGB(255,201,255,255);
int NoSmoke;
int NoFlash;
int WhiteWalls;
int AsusWalls;
...
if(xhair)
{
D3DRECT rec4 = {ScreenCenterX-20, ScreenCenterY, ScreenCenterX+ 20, ScreenCenterY+1};
...
D3DRECT rec6 = {ScreenCenterX-20, ScreenCenterY, ScreenCenterX+ 20, ScreenCenterY+1};
...
D3DRECT rec8 = {ScreenCenterX-20, ScreenCenterY, ScreenCenterX+ 20, ScreenCenterY+1};
...
Device_Interface->Clear(1, &rec2, D3DCLEAR_TARGET,lightSteelBluet, 0,  0);
  Device_Interface->Clear(1, &rec3, D3DCLEAR_TARGET,lightSteelBluet, 0,  0);
} 
   /*
/*
...
pMenu->AddGroup("[ChamColors 1]" , &CH_Chams, opt_Grp);
pMenu->AddGroup("[ChamColors 2]" , &CH_Chams2, opt_Grp);
...
pMenu->AddItem("R-bow Xhair" , &xhair , opt_OffOn);
pMenu->AddItem("Xhair 2" , &xhair2 , opt_OffOn);
...
pMenu->AddItem("AsusWall" , &AsusWalls , opt_OffOn);
...
pMenu = new D3DMenu("Trismund's Hack",100,150); // title, maxitems,width
...
{
     BYTE *jmp = (BYTE*)malloc(len+5); //allocate memory on heap for the codecave
    DWORD dwback;
    VirtualProtect(src, len, PAGE_READWRITE, &dwback); //remove page protection and save the old
    memcpy(jmp, src, len);  //copy the bytes you gonna overwrite from the orginal func to the codecave 
    jmp += len; //increase the jmp pointer so it points after the bytes we copied already in the codecave
    jmp[0] = 0xE9; //write an jmp instruction to that place
    *(DWORD*)(jmp+1) = (DWORD)(src+len - jmp) - 5; //write the relative jump offset which leads back to our orginal function
    memset( src, 0x90, len ); //set the orginal function prolog to 0x90
    src[0] = 0xE9; //write an jmp instruction at the start of the source function
    *(DWORD*)(src+1) = (DWORD)(dst - src) - 5; //write the relative jump offset which leads to our hooked function ( aka destinatio )
    VirtualProtect(src, len, dwback, &dwback); //set page protection back to normal
    return (jmp-len); //return the addy of the code cave  
}
