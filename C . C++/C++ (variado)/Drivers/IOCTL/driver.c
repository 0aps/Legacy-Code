...
NTSTATUS DDKAPI Example_IoControl(PDEVICE_OBJECT DeviceObject,PIRP Irp){
  ...
  PCHAR pDataInput,pDataOutput;
  UINT dwDataReaded=0,dwDataWritten=0;
  ...
  if(pDriverBuffer&&pIoStackIrp){
    ...
    switch(pIoStackIrp->Parameters.DeviceIoControl.IoControlCode){
      ...
      case IOCTL_EJEMPLO_DIRECT_IN_IO:
        if(Irp->MdlAddress){
          //los nombres de las variables son incorrectos dados que ambos son entradas
          pDataInput=Irp->AssociatedIrp.SystemBuffer;
          pDataOutput=MmGetSystemAddressForMdlSafe(Irp->MdlAddress, NormalPagePriority);

          if(pDataInput&&pDataOutput){

            if(pIoStackIrp->Parameters.DeviceIoControl.OutputBufferLength < BUFFERSIZE)
              dwDataWritten=pIoStackIrp->Parameters.DeviceIoControl.OutputBufferLength;

            else
              dwDataWritten=BUFFERSIZE;

            //cuidado con esto! debe ser una cadena terminada en NULL!
            DbgPrint("UserModeMessage = '%s' Size %d",pDataInput,pIoStackIrp->Parameters.DeviceIoControl.InputBufferLength);

            RtlCopyMemory(pDriverBuffer,pDataOutput,dwDataWritten);
            DbgPrint("IOCTL_METHOD_IN_DIRECT -> Writen %d Bytes \r\n",dwDataWritten);
            NtStatus=STATUS_SUCCESS;
          }
        }
      break;

      case IOCTL_EJEMPLO_DIRECT_OUT_IO:
        if(Irp->MdlAddress){
          ...
          if(pDataInput&&pDataOutput){
            ...
            if(pIoStackIrp->Parameters.DeviceIoControl.OutputBufferLength < BUFFERSIZE)
              dwDataReaded=pIoStackIrp->Parameters.DeviceIoControl.OutputBufferLength;
            ...
            RtlCopyMemory(pDataOutput,pDriverBuffer,dwDataReaded);
            DbgPrint("IOCTL_METHOD_OUT_DIRECT -> Readed %d Bytes \r\n",dwDataReaded);
            NtStatus=STATUS_SUCCESS;
          }
        }
      break;
      case IOCTL_EJEMPLO_BUFFERED_IO:

        pDataInput=Irp->AssociatedIrp.SystemBuffer;
        pDataOutput=Irp->AssociatedIrp.SystemBuffer;
        ...
        if(pDataInput){
          ...
          RtlCopyMemory(pDataOutput,pDriverBuffer,dwDataReaded);
          DbgPrint("IOCTL_METHOD_BUFFERED -> Readed %d Bytes \r\n",dwDataReaded);
          NtStatus=STATUS_SUCCESS;
        }
      break;

      case IOCTL_EJEMPLO_NEITHER_IO:

        //hay que tener cuidado con este método ya que el acceso es directo!

        pDataInput=pIoStackIrp->Parameters.DeviceIoControl.Type3InputBuffer;
        pDataOutput=Irp->UserBuffer;
        ...
        if(pDataInput&&pDataOutput){
          ...
          RtlCopyMemory(pDataOutput,pDriverBuffer,dwDataReaded);
          DbgPrint("IOCTL_METHOD_NEITHER -> Readed %d Bytes \r\n",dwDataReaded);
          NtStatus = STATUS_SUCCESS;
        }
      break;
    }
